...
exports.WardleyMapRenderer = void 0;
const canvas_1 = require("canvas");
class WardleyMapRenderer {
    constructor(width = 1400, height = 1000) {
        this.width = width;
        this.height = height;
    ...
    getComponentColor(component) {
        ...
        if (component.category === 'build')
            return '#4A90E2';
        if (component.category === 'buy')
            return '#7ED321';
        if (component.category === 'outsource')
            return '#F5A623';
        return '#000000';
    }
    drawEvolutionAxis() {
    ...
    drawConnection(connection, map) {
        ...
        this.ctx.lineTo(toX - arrowLength * Math.cos(angle - Math.PI / 6), toY - arrowLength * Math.sin(angle - Math.PI / 6));
        this.ctx.moveTo(toX, toY);
        this.ctx.lineTo(toX - arrowLength * Math.cos(angle + Math.PI / 6), toY - arrowLength * Math.sin(angle + Math.PI / 6));
        this.ctx.stroke();
    }
...
exports.WardleyMapRenderer = WardleyMapRenderer;
